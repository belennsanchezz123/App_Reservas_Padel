...
└── Documentacion/      # Toda la documentación .md
    ...
    ├── UX.md                   # Patrones de interacción (scroll, sheets, gestos) reutilizables
...
- **Monitor**: gestiona únicamente sus propias clases y alumnos. En el detalle de una clase, cada alumno inscrito tiene dos acciones (`showClassDetails`): **"🔁 Ausente"** (`markAbsence`) genera una clase por recuperar **sin sacarlo de la clase** (no vino pero sigue inscrito), y **"✕"** (`removeStudentFromClass`) **sí lo saca de la clase** y libera la plaza (p. ej. avisó de que no puede venir). El monitor solo pulsa la ✕ (una confirmación) y el sistema decide solo: si el alumno **había pagado** la plaza (`class_requests.confirmada_pagada`, comprobado con `db.getPaidRequestForClass`) se le genera una **clase por recuperar** automáticamente; si no había pagado (añadido a mano), la plaza simplemente se pierde. Quitar al alumno usa `updateClass` (RLS: el monitor edita sus propias clases). Limitación conocida: la fila `class_requests` pagada NO se cierra (RLS solo deja al cliente la transición `pendiente→rechazada`); quedaría como `confirmada_pagada` histórica. Cerrarla + reembolso requeriría un endpoint de servicio. En su vista de calendario tiene el botón **"📩 Solicitudes"** (con contador de pendientes) que abre el apartado de solicitudes de inscripción de sus clases: acepta (`acceptRequest`) o rechaza (`rejectRequest`). **Aceptar no inscribe al alumno: genera su link de pago** (Stripe Checkout) y retiene la plaza; el alumno aparece en el calendario cuando paga (aviso en vivo por Realtime). Bajo las solicitudes pendientes, el modal lista las que están "Esperando pago" con su cuenta atrás. El aforo que ve (`2+1/4`) ya incluye las plazas retenidas. Junto a Solicitudes tiene la **"🔔 Avisos"** (bandeja de notificaciones, `openNotifModal`/`renderNotifBadge`): los toasts en vivo son efímeros, así que la bandeja lista los avisos de la tabla `notifications` (pagos recibidos, bajas de alumnos, nuevas solicitudes) con contador de no leídos; al abrirla se marcan todos como leídos (`markAllNotificationsRead`), cada aviso tiene un ✕ para borrarlo (`deleteNotif`) y hay un "Borrar todos" con confirmación (`clearAllNotifs`). La campana se adapta al layout (misma bandeja y contador en ambos): en **escritorio** es el botón ancho `#notifBellBtn` (solo se muestra si `!isMobileLayout()`); en **móvil** es un icono flotante 🔔 del tamaño de la lupa, **a su izquierda**, en la barra `month-top-actions` (`#notifBellBtnMobile`). `renderNotifBadge` actualiza los dos badges y decide su visibilidad. **Importante**: la toolbar móvil se construye UNA sola vez (`if (!container)` en `renderMonthCalendar`) y puede montarse en el primer `renderCalendar()` de `initializeApp` cuando `currentUser` aún es `null`; por eso el icono 🔔 se crea SIEMPRE oculto y es `renderNotifBadge` (llamada tras el login) quien lo muestra si el usuario es monitor. No volver a condicionar su creación a `isMonitor()`. El borrado necesita la política RLS `notifications_delete` (el destinatario borra solo los suyos, `rls_activacion_final.sql`). Así una baja ocurrida con la app cerrada no se pierde.
